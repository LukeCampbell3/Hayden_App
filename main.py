...
def separate_audio(input_audio_path, output_directory):
    ...
    # Paths for the separated files before noise reduction
    ...
    # Write the processed and resampled vocals back to file
    sf.write(vocals_path, vocals_audio_resampled, 16000)

    # Move and rename the separated (and processed) files to the final location
    vocals_path = os.path.join(output_directory, f"{file_name_without_extension} vocals.wav")
    os.rename(vocals_path, vocals_path)
    
    # Clean up the folder created by Spleeter and the other unused stems
    # os.remove(bass_path)
    # os.remove(drums_path)
    # os.remove(other_path)
    # os.rmdir(separated_folder)
